...
class ItemRef(object):
    ...
    Do not use this class directly; Only use instances returned
    BinomialHeap.insert()
    ...
    def in_heap(self, heap):
        """Returns True if the referenced item is part of the BinomialHeap
...
class BinomialHeap(object):
    ...
    def insert(self, key, value=None):
        """Insert 'value' in to the heap with priority 'key'. If 'value' is
        omitted, then 'key' is used as the value.
    ...
    def min(self):
        """Returns the value with the minimum key (= highest priority) in the
        heap without removing it, or None if the heap is empty.
        """
        pos = self.__min()
    ...
    def extract_min(self):
        """Returns the value with the minimum key (= highest priority) in the
        heap AND removes it from the heap, or None if the heap is empty.
        """
        # find mininum
        pos = self.__min()
        if not pos:
    ...
    def next(self):
        """Returns the value with the minimum key (= highest priority) in the
        heap AND removes it from the heap; raises StopIteration if the heap is
        empty.
        """
        if self.head:
