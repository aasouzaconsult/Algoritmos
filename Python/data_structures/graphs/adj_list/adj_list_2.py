...
class Graph(object):
    ...
    def bfs(self, start, callback=print_vertice):
        ...
        visited[start] = True
        print start,
        # put start adj vertices in queue
        ...
        while len(queue) > 0:
            ...
            vertice = queue.pop(0)
            # if not visited, visit vertice and append adj vertices to queue
    ...
    def dfs(self, start, callback=print_vertice):
        ...
        visited[start] = True
        print start,
        # put start adj vertices in stack
        curr = self.graph[start].get_head()
        ...
        # process stack
        while len(stack) > 0:
            # pop most recent vertice
            vertice = stack.pop()
            # if not visited, visit vertice and append adj vertices to stack
            if not visited[vertice.data]:
                visited[vertice.data] = True
                callback(self, vertice)
            # append adj vertices to stack
            curr = self.graph[vertice.data].get_head()
            aux_stack = []
            while curr is not None:
