...
public class Newton {
    ...
    public static void main (String args[])
    {
        ...
        try
        {
        	...
        	double y1,y2;
        	y1=6*d1-6;  // Substituindo x por d1 
        	y2=6*d2-6;  // Substituindo x por d2
                
        	double x[] = new double [100];
        	double f1x[] = new double [100];
        	double f2x[] = new double [100];
        
        	System.out.println("\n* Valores de X tendendo ao Otimo *"); 
        	for ( int k = 1 ; k <= n ; k++ )
	    	{
            	...
            	// Calculo das funções para Derivada 1º ( 3x^2 - 6x - 9 = 0 )
            	f1x[k]= 3*(x[k]*x[k])- 6*x[k] - 9;
            
            	// Calculo das funções para Derivada 2º ( 6x - 6 = 0 )
            	f2x[k]= 6*x[k]- 6;   
          	}
